...
export default {
    computed: {
      IdentityModel() {
        ...
        SelectedSkill(){
            return this.$store.getters['skillStore/SelectedSkill'];
        },
        base64String(){
            return this.$store.getters['skillStore/base64String'];
        }
      },
      components: {
    ...
    return {
      ...
      methods: {
        getUrl (files) {
          return `http://localhost:52766/api/Skills/UploadPhoto`
        },
          file_selected(file) {
              var context = this;
            ...
            uploadFile(){
                ...
                this.$axios.post('Skills/UploadPhoto',fd,{
          ...
          this.$store.dispatch('skillStore/UpdateSkill', {
                statement: context.statement,
                base64String:context.base64String,
                id: context.SelectedSkill.id,
      ...
      cancel(){
        this.$store.commit('skillStore/CancelSkillDataUDialog');
      }
    },
    created() {
        var context = this;
        var skill = this.$store.getters['skillStore/SelectedSkill']
        context.statement = skill.statement;
