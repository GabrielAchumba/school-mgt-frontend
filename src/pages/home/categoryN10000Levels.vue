  ...
  <div>
      <div class="row bg-primary">
            <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
              <h5 class="text-center text-accent">Category ₦10000.00
                <br>
              </h5>
            </div>
          </div> 
  ...
  import CardList from "../../components/Cards/CardList.vue"
    ...
    export default {
      ...
      async created(){
        ...
        if(success){
          let sum = 0;
          context.n10000ROIs = n10000ROIs.map((n10000ROI, index) => {
            const downLiners = Math.pow(3,(index+1));
            sum = sum + Number(n10000ROI.replaceAll(',', ''))
            return {
                title: `Level ${index + 1} `, 
                description: `Contributor (Category ₦10000.00) receives ₦${n10000ROI} after completing Level ${index+1}. This Level has ${downLiners} downliners.`,
                image:  `/statics/images/${context.images[index]}.png`,
              }
          });

          var len = context.images.length - 1;
          context.n10000ROIs.push({
                title: "Total", 
